...
def process_meme_templates(page: int, data_dir: str, save_dir: str, n_pages_per_meme: int, num_attempts: int) -> Optional[DataFrame]:
  ...
  with ThreadPoolExecutor() as executor:
    futures = [executor.submit(lambda x: process_meme_template(x, data_dir, save_dir, n_pages_per_meme, num_attempts), meme_template) for meme_template in get_bs(MEME_TEMPLATES_URL, payload={'page': str(page + 1)}, parse_only=ss(class_='mt-title'))]
...
def main(n_pages_meme_types: int, n_pages_per_meme: int, data_dir: str, save_dir: str, outfile: str, num_attempts: int) -> None:
  ...
  for i in range(n_pages_meme_types):
    dfs.append(process_meme_templates(i, data_dir, save_dir, n_pages_per_meme, num_attempts))
  if len(dfs) == 0:
...
if __name__ == "__main__":
  ...
  parser.add_argument('-d', '--data_dir', type=str, default='data', help='local directory name where data should be saved')
  parser.add_argument('-s', '--save_dir', type=str, default='meme_templates', help='name of directory to save meme templates for which captions were found')
  parser.add_argument('-o', '--outfile', type=str, default='data.tsv', help='TSV filename for scraped captions and metadata')
  ...
  main(args.n_pages_meme_types, args.n_pages_per_meme, args.data_dir, args.save_dir, args.outfile, args.num_attempts)
